...
class HCA_Mesh_Generator:
    ...
    def generate_mesh(self):
        ...
        # 更新单元ID映射表
        self.eid_map[3 * self.n_r - 1: 3*self.n_r+1] = np.array([3 * self.n_r - 2,3*self.n_r-1])
        # 添加进单元集
        ...
        # 更新单元ID映射表
        self.eid_map[3*self.n_r+2:3*self.n_r+4] = np.array([3*self.n_r+1, 3*self.n_r+2])
        # 分类
    ...
    def rotate_mesh(self, target_elset: list):
        ...
        for itheta in range(1, self.n_theta):
            ...
            self.nid_map[new_nids] = np.arange(n_start_idx, n_start_idx + len(new_coords))
    ...
    def mesh_plot(self, show_labels=True, label_ratio=1.0):
        ...
        mesh = pv.UnstructuredGrid(cells.flatten(), 
        ...
        if show_labels:
            ...
            plotter.add_point_labels(node_coords, node_labels, font_size=12, 
                                     text_color="red", name="nodes", always_visible=False)
            ...
            plotter.add_point_labels(centers, elem_labels, font_size=10, 
                                     text_color="green", name="elems", shadow=True)

        plotter.add_legend(labels=[('Node ID', 'red'), ('Elem ID', 'green')], bcolor=None)
    ...
    def export_to_inp(self, filename="HCA_Model.inp"):
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            # 网格信息
            f.write(f"Information of Mesh: Node = {self.node.shape[0]}, Element = {self.conn.shape[0]}\n")
    ...
    def export_to_inp1(self, filename="HCA_Model.inp"):
        """
        根据 elset 的分类自动分配单元类型并导出组件
        """
        # 定义关键字映射：只要键名包含这些字符，就应用对应的 Abaqus 单元类型
        TYPE_MAP = {
            "beam": "B31",            # 梁单元，带抗弯刚度
            "surface_cable": "T3D2",  # 索单元，仅受拉压
            "support_cable": "T3D2",  # 支撑索
        }
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            # 2. 递归解析 elset 并写入对应的 *ELEMENT 块
            def write_categorized_components(elset_dict, prefix="", current_type="T3D2"):
                for name, content in elset_dict.items():
                    # 更新当前分支应使用的单元类型
                    # 比如进入 "beam" 分支，类型变为 B31
                    ...
                    if isinstance(content, dict):
                        # 递归向下，传递当前分支确定的单元类型
                        write_categorized_components(content, prefix=f"{base_name}_", current_type=branch_type)
                    
                    elif isinstance(content, list):
                        if not content: continue
                        
                        # 处理嵌套列表 (如 RADIAL-1, RADIAL-2)
                        if isinstance(content[0], list):
                            for i, sub_list in enumerate(content):
                                if sub_list:
                                    set_name = f"{base_name}-{i+1}"
                                    self._write_element_block(f, set_name, sub_list, branch_type, written_eids)
                        else:
                            # 处理普通列表
                            self._write_element_block(f, base_name, content, branch_type, written_eids)

            # 开始从根部解析
            write_categorized_components(self.elset)
            
        print(f"成功导出分类单元 Inp 文件: {filename}")

    def _write_element_block(self, file_handle, set_name, eid_list, element_type, written_set):
        """写入带类型的 *ELEMENT 块，并更新已写入集合"""
        # 排除已处理的单元
        valid_eids = [eid for eid in eid_list if eid not in written_set]
        if not valid_eids:
            return

        file_handle.write(f"*Element, type={element_type}, elset={set_name}\n")
        for eid in valid_eids:
            p_idx = self.eid_map[eid]
            nodes = self.conn[p_idx]
            file_handle.write(f"{eid}, {nodes[0]}, {nodes[1]}\n")
            written_set.add(eid)

if __name__ == "__main__":
    # 测试代码
    generator = HCA_Mesh_Generator(n_r=4, n_theta=6)
