...
class FDMGA1Problem(ElementwiseProblem):
    def __init__(self, solver, q_init, q_range_ratio, elset_indices,
                 weights=(0.5, 0.5), # (w_rmse, w_ratio)
                 rms_bounds=(0.0, 2.0), # 用于归一化的 RMSE 范围 [min, max]
                 t_limits=None, u_limit=None, use_penalty=True):

        n_groups = solver.num_group
        super().__init__(n_var=n_groups, n_obj=1, 
                         xl=q_range_ratio[0], xu=q_range_ratio[1])

        self.solver = solver
        self.q_init = q_init
        self.cable_elset = elset_indices
        self.weights = weights
        self.rms_bounds = rms_bounds
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        try:
            ...
            # 2. 归一化评价 (你的公式实现)
            # f1: RMSE 归一化
            r_min, r_max = self.rms_bounds
            f1 = (rmse - r_min) / (r_max - r_min + 1e-8)
            ...
            # 总目标：加权求和
            w_rmse, w_ratio = self.weights
            f = w_rmse * f1 + w_ratio * f2
...
class FDMOptimizer:
    ...
    def run_GA1(self, q_init, q_ratio_bounds=(0.8, 1.2), t_limits=(10.0, 500.0), 
                u_limit=70.0, pop_size=100, n_gen=100, 
                weights=(0.5, 0.5), rms_bounds=(0.0, 2.0), penalty=True):
        """
        支持归一化加权评价的单目标 GA 精修
        weights: (w_rmse, w_ratio)
        rms_bounds: (rms_min, rms_max) 用于归一化的参考区间
        """
        print(f"\n>> GA1 精修开始 | 权重 (RMSE:{weights[0]}, Ratio:{weights[1]}) | 半径: {q_ratio_bounds}")
        ...
        problem = FDMGA1Problem(
            ...
            u_limit=u_limit, weights=weights, rms_bounds=rms_bounds,
            use_penalty=penalty
        )
